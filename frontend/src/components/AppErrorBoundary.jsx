    ...
    return (
      <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24, background: '#f6fbfa', color: '#17352f' }}>
        <section role="alert" style={{ width: 'min(100%, 560px)', padding: 32, borderRadius: 24, background: '#fff', border: '1px solid #d9eee8', boxShadow: '0 18px 50px rgba(26, 106, 88, 0.12)', textAlign: 'center' }}>
          ...
          <h1 style={{ margin: '0 0 12px', fontSize: 24 }}>잠시 문제가 발생했습니다</h1>
          <p style={{ margin: '0 auto 20px', lineHeight: 1.7, color: '#53706a' }}>화면을 안전하게 복구할 수 있습니다. 다시 시도하거나 첫 화면으로 이동해주세요.</p>
          <p style={{ margin: '0 0 20px', fontSize: 12, color: '#8aa39d', fontFamily: 'monospace' }}>오류 ID: {this.state.errorId}</p>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 10, flexWrap: 'wrap' }}>
            <button type="button" onClick={() => window.location.reload()} style={{ border: 0, borderRadius: 999, padding: '11px 18px', background: '#30c59b', color: '#fff', fontWeight: 700, cursor: 'pointer' }}>다시 시도</button>
            <button type="button" onClick={() => window.location.assign('/')} style={{ border: '1px solid #b9ded4', borderRadius: 999, padding: '11px 18px', background: '#fff', color: '#237c68', fontWeight: 700, cursor: 'pointer' }}>첫 화면으로</button>
          </div>
        </section>
      </main>
    );
