...
const InterviewSession = () => {
  ...
  const completeInterview = async () => {
    ...
    try {
      ...
      setError(completionError.message || copy.completeFailed);
    } finally {
      setUploading(false);
    }
  };
  ...
  const retryCurrentUpload = () => {
    if (pendingCompletionRef.current && !uploading) {
      setError('');
      completeInterview();
      return;
    }
        ...
        <div className="interview-session-question-panel" style={{ flex: 1, padding: 40, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f8f9fa', position: 'relative' }}>
          ...
          {error && (
            <div style={{ color: '#e74c3c', marginTop: 18, fontWeight: 600, textAlign: 'center' }}>
              ...
              >
                {pendingCompletionRef.current ? copy.retryCompletion : pendingBlobRef.current ? copy.retryUpload : copy.retrySession}
              </button>
            </div>
          )}
        </div>
