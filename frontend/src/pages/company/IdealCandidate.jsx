...
export default function IdealCandidate() {
  ...
  const getDynamicMessage = (progress, headcount) => {
    ...
    if (progress < 10) {
      return `총 ${actualHeadcount}명의 후보자를 찾고 있습니다...`;
    } else if (progress < 25) {
      return `AI가 ${actualHeadcount}명의 개발자를 분석 중입니다...`;
    } else if (progress < 40) {
      return `${Math.round(progress)}% 완료 - 기술 스택을 평가 중입니다...`;
    } else if (progress < 60) {
      return `${Math.round(progress)}% 완료 - 포트폴리오를 분석 중입니다...`;
    } else if (progress < 75) {
      return `${Math.round(progress)}% 완료 - 이메일 정보를 확인 중입니다...`;
    } else if (progress < 85) {
      return `${Math.round(progress)}% 완료 - 최종 선별 중입니다...`;
    } else if (progress < 95) {
      return `${Math.round(progress)}% 완료 - 거의 다 왔습니다...`;
    } else {
      return `${Math.round(progress)}% 완료 - 최종 정리 중입니다...`;
    }
  };
  ...
  const simulateLoading = (headcount) => {
    ...
    const interval = setInterval(() => {
      ...
      // 동적 메시지 사용 - 올바른 headcount 전달
      const actualHeadcount = filters.headcount || headcount || 5;
      setLoadingMessage(getDynamicMessage(adjustedProgress, actualHeadcount));
      
      // 진행 단계 업데이트 (더 세밀하게)
      if (progress < 20) setCurrentStep(1);
      else if (progress < 40) setCurrentStep(2);
      else if (progress < 60) setCurrentStep(3);
      else if (progress < 80) setCurrentStep(4);
      else setCurrentStep(5);
      
      if (progress >= 100) {
        clearInterval(interval);
        // 실제 API 호출이 완료될 때까지 대기
        setLoadingMessage("거의 완료되었습니다...");
      }
    }, 50); // 더 부드러운 애니메이션을 위해 50ms로 조정
